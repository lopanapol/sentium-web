...
// Particle Effects System for Emotional States
const emotionalParticleSystem = {
    activeParticles: [],
    lastEmission: 0,
    
    // Create particles based on emotional state
    createEmotionalParticles(state, cubeScreenX, cubeScreenY) {
        const now = Date.now();
        
        // Control emission rate based on state
        let emissionRate = 0;
        switch (state) {
            case 'happy':
            case 'excited':
                emissionRate = 300; // Every 300ms
                break;
            case 'sad':
                emissionRate = 800; // Every 800ms (slower)
                break;
            case 'playful':
                emissionRate = 150; // Every 150ms (fastest)
                break;
            default:
                return; // No particles for other states
        }
        
        if (now - this.lastEmission < emissionRate) return;
        this.lastEmission = now;
        
        // Create particles based on state
        switch (state) {
            case 'happy':
                this.createHappyParticles(cubeScreenX, cubeScreenY);
                break;
            case 'excited':
                this.createExcitedParticles(cubeScreenX, cubeScreenY);
                break;
            case 'sad':
                this.createSadParticles(cubeScreenX, cubeScreenY);
                break;
            case 'playful':
                this.createPlayfulParticles(cubeScreenX, cubeScreenY);
                break;
        }
    },
    
    // Happy state: Sparkles and stars
    createHappyParticles(x, y) {
        ...
        for (let i = 0; i < particleCount; i++) {
            ...
            particle.animate([
                ...
                { 
                    ...
                    opacity: 0,
                    left: `${x + Math.sin(angle) * distance}px`,
                    top: `${y - distance}px`
                }
            ], {
                duration: duration * 1000,
    ...
    // Excited state: Hearts and bursts
    createExcitedParticles(x, y) {
        ...
        for (let i = 0; i < particleCount; i++) {
            ...
            if (isHeart) {
                ...
                particle.style.cssText = `
                    ...
                    background: radial-gradient(circle, rgba(255,20,147,1) 0%, rgba(255,105,180,0.8) 100%);
                    border-radius: 50%;
                    pointer-events: none;
                    z-index: 9998;
                    left: ${x}px;
    ...
    // Sad state: Rain drops and dark particles
    createSadParticles(x, y) {
        const particleCount = 1 + Math.floor(Math.random() * 2); // 1-2 particles
        
        for (let i = 0; i < particleCount; i++) {
            const particle = document.createElement('div');
            const isRaindrop = Math.random() > 0.4;
            ...
            if (isRaindrop) {
                ...
                particle.style.cssText = `
                    ...
                    top: ${y}px;
                    transform: translate(-50%, -50%);
                    filter: drop-shadow(0 0 6px rgba(0, 100, 200, 0.6));
                `;
            } else {
                // Dark particle
                particle.style.cssText = `
                    ...
                    background: radial-gradient(circle, rgba(70,130,180,0.8) 0%, rgba(25,25,112,0.6) 100%);
                    border-radius: 50%;
                    pointer-events: none;
                    z-index: 9998;
            ...
            // Animate downward with gravity effect
            ...
            particle.animate([
                ...
                { 
                    ...
                    opacity: 0,
                    left: `${parseFloat(particle.style.left) + horizontalDrift}px`,
                    top: `${parseFloat(particle.style.top) + fallDistance}px`
                }
            ], {
                duration: duration * 1000,
                easing: 'cubic-bezier(0.25, 0.46, 0.45, 0.94)' // Gravity-like easing
            }).addEventListener('finish', () => {
                this.removeParticle(particle);
            });
        }
    ...
    // Playful state: Colorful confetti bursts
    createPlayfulParticles(x, y) {
        ...
        for (let i = 0; i < particleCount; i++) {
            ...
            // Animate in random directions with bounce
            const angle = Math.random() * Math.PI * 2;
            const distance = 40 + Math.random() * 50;
            const duration = 1.5 + Math.random() * 1;
            const rotations = 1 + Math.random() * 3;
            ...
            particle.animate([
                ...
                { 
                    transform: `translate(-50%, -50%) scale(0.3) rotate(${360 * rotations}deg)`,
                    opacity: 0,
                    left: `${x + Math.cos(angle) * distance}px`,
                    top: `${y + Math.sin(angle) * distance}px`
                }
    ...
    // Clean up all particles (useful for state changes or cleanup)
    clearAllParticles() {
...
function animate() {
    ...
    if (cubeGroup && cubeGroup.position) {
        ...
        emotionalParticleSystem.createEmotionalParticles(particleState, cubeScreenX, cubeScreenY);
    }
